...
void GetVersionReg(OSVERSIONINFO *oOS_version)
{
	...
	// Get version
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion", 0, KEY_ALL_ACCESS, &RegKey) == ERROR_SUCCESS)
	{
		if (RegQueryValueEx(RegKey, L"CurrentMajorVersionNumber", nullptr, &dwType, (LPBYTE)&dwDataMajor, &iSize) == ERROR_SUCCESS &&
			RegQueryValueEx(RegKey, L"CurrentMinorVersionNumber", nullptr, &dwType, (LPBYTE)&dwDataMinor, &iSize) == ERROR_SUCCESS)
		{
...
void GetVersionFile(OSVERSIONINFO *oOS_version)
{
	...
	// Load version.dll
	HMODULE Module = LoadLibrary(L"version.dll");
	if (!Module)
	{
	...
	wcscat_s(buffer, MAX_PATH, L"\\kernel32.dll");

	// Define registry keys
	DWORD  verHandle = 0;
	UINT   size = 0;
	LPBYTE lpBuffer = nullptr;
	LPCWSTR szVersionFile = buffer;
	...
	if (verSize != 0)
	{
		...
		{
			if (VerQueryValue(&verData[0], L"\\", (VOID FAR* FAR*)&lpBuffer, &size))
			{
				if (size)
				{
...
void LogOSVersion()
{
	...
	const wchar_t *ServicePack = ws.c_str();
	...
	// Get OS string name
	wchar_t *sOSName = L"Unknown platform";
	if (IsWindowsServer())
	{
		sOSName = L"Unknown Windows Server";
		...
		case 5:
			...
			{
			case 0:
				sOSName = L"Windows 2000 Server";
				break;
			case 2:
				sOSName = (GetSystemMetrics(SM_SERVERR2) == 0) ? L"Windows Server 2003" : L"Windows Server 2003 R2";
				break;
			}
			break;
		case 6:
			...
			{
			case 0:
				sOSName = L"Windows Server 2008";
				break;
			case 1:
				sOSName = L"Windows Server 2008 R2";
				break;
			case 2:
				sOSName = L"Windows Server 2012";
				break;
			case 3:
				sOSName = L"Windows Server 2012 R2";
				break;
			}
			break;
		case 10:
	...
	{
		sOSName = L"Unknown Windows Desktop";
		switch (oOS_version.dwMajorVersion)
		{
		case 5:
			switch (oOS_version.dwMinorVersion)
			{
			case 0:
				sOSName = L"Windows 2000";
				break;
			case 1:
				sOSName = L"Windows XP";
				break;
			case 2:
				sOSName = L"Windows XP Professional"; // Windows XP Professional x64
				break;
			}
			break;
		case 6:
...
void LogComputerManufacturer()
{
	...
	if ((hr = RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\Services\\mssmbios\\Data", 0, KEY_QUERY_VALUE, &hkData)) != ERROR_SUCCESS)
	{
		return;
	}

	if ((hr = RegQueryValueEx(hkData, L"SMBiosData", nullptr, &dwType, nullptr, &dwSize)) == ERROR_SUCCESS)
	{
		...
		else
		{
			...
			{
				hr = RegQueryValueEx(hkData, L"SMBiosData", nullptr, nullptr, lpData, &dwSize);
			}
		}
	}
